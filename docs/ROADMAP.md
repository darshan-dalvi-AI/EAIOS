...
- ~~Deferred: CRDT (Yjs) shared-notes app (stretch).~~ → **shipped 2026-08-01** as the Code app (see the collaborative-coding batch below) — Yjs CRDT over `/api/ws/collab/{file_id}`, several people editing one file at once.
...
Four platform features that shift K-OS from "AI demo" toward "enterprise platform":
...
- **HR role** — fourth RBAC role for people operations: hire/manage line staff (managers/employees) via a scoped "HR console" (Users + Access tabs only); can't touch model keys, admin/HR accounts, or grant admin. Enforced by `require_admin_or_hr` + assignment guardrails. Seeded demo user `hr@eaios.dev`.
...
Turns K-OS from one deployment into a product any company can self-serve — one hosted app, isolated workspaces per company.
...
## Collaborative coding · ✅ shipped 2026-08-01

- **Code app (19th)** — Monaco editor with a project/file tree, editor tabs, syntax highlighting and version history. Monaco is **bundled, not CDN-loaded**: the app's CSP is `script-src 'self'`, so a CDN loader would be blocked outright. It sits in its own lazy chunk (~3.9 MB) that downloads only when the app is opened.
- **Simultaneous editing (Yjs CRDT)** — `core/collab.py` keeps one room per file over `/api/ws/collab/{file_id}`, relays opaque binary updates between editors and persists the merged state on a 3s debounce. The server never parses or merges the document; convergence is a property of the CRDT, not the transport, so updates may arrive in any order and every client still lands on identical text. Peer cursors ride the same socket as awareness frames. Proven by a test driving **five concurrent editors**.
- **Version control** — `services/vcs.py`, git's model in Postgres: content-addressed blobs, snapshot commits, a parent chain, branches, on-demand diffs, and restore that first auto-commits uncommitted work to a `rescue-*` branch so nothing is silently discarded. Merging is deliberately absent. Blob ids are salted with the workspace (`sha256(org_id ‖ content)`) — a plain content hash is a global key, but blobs are tenant rows, so two workspaces committing identical boilerplate would otherwise collide on that key and break each other's commits.
- **AI coding assistant** — explain / find bugs / write tests / document / refactor, scoped to the selection. It **bypasses the Coding Agent's retrieval step on purpose**: routed through RAG, "explain this function" came back quoting a liability clause from a contract in the knowledge base.
- **Industry-adaptive workspaces** — `services/industries.py` gives each field its own app set; the dock shows those and keeps the rest in an "All apps" drawer.
- **In-browser code execution** — Python (CPython→WebAssembly via a pinned Pyodide) and JavaScript run in an iframe sandboxed to an **opaque origin** (`allow-scripts` *without* `allow-same-origin`), inside a terminable blob Worker. No cookies, no storage, no parent DOM, CORS-blocked from the API; runaway programs are killed by a timeout with the boot download on a separate clock. Server-side execution was considered and **refused**: it is remote code execution on a shared container. The threat is a colleague, not a stranger — the editor is shared, so a same-origin Worker would have run their code with your session.
- Verified: **385 backend pytest** (incl. 14 code-runner tests asserting the sandbox is granted no `'self'` in any CSP directive, only the pinned CDN is reachable, and execution happens in a terminable worker) · tsc clean · vite build clean · live-browser QA against production — frame origin `"null"`, `document.cookie` / `localStorage` / `parent.document` / `fetch('/api/health')` all fail, `while(true){}` killed on time with the tab responsive, real Python run returning `Python 3.13.2` and numpy auto-installed.
- Fixed during that live QA: an `IndexError` printed nine traceback lines, six of them inside Pyodide's `_base.py`; tracebacks are now trimmed to the user's own frames and named with their filename.
