/* "Download app" button — installs K-OS as a standalone app (PWA).
...
export default function InstallButton({ className = "btn", label = "Download app" }: { className?: string; label?: string }) {
  ...
  return (
    <>
      <button type="button" className={className} onClick={click} title="Install K-OS as an app on your device">
        ...
        <div className="tour-overlay" style={{ background: "rgba(2,4,10,.6)", zIndex: 100002, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setHelp(false)} role="dialog" aria-modal="true">
          <div className="tour-card" style={{ maxWidth: 380 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              ...
              <b style={{ fontSize: 15 }}>Install K-OS as an app</b>
            ...
            {isIOS() ? (
              <p style={{ fontSize: 12.5, lineHeight: 1.6, color: "var(--text-dim)", margin: 0 }}>
                ...
                then choose <b>“Add to Home Screen”</b>. K-OS opens like a native app — no browser bar.
              </p>
            ) : (
              <p style={{ fontSize: 12.5, lineHeight: 1.6, color: "var(--text-dim)", margin: 0 }}>
                <Monitor size={13} /> On Chrome or Edge (Windows/Mac/Android): open the browser menu (or the
                install icon in the address bar) and choose <b>“Install K-OS”</b>. It gets a desktop/home-screen
